...
Kullanıcı bir nesnenin 3-8 fotoğrafını yükler; her fotoğraftan iki parmak izi
çıkarılır:
...
Karar ikisinin BİRLEŞİMİDİR ve iki kanıt birbirini doğrulamalıdır. Yöntemin
sınırı açıkça söylenir: ayırt edici deseni/rengi olan nesnelerde (yazılı pano,
sarı tüp, markalı kutu) iyi çalışır; düz gri desensiz nesnelerde zayıftır.
Eşiğin altında kalan en iyi skor bile kabul edilmez — sistem uydurma isim
YAZMAZ, "eşleşme yok" der (docs/12'deki üç durum ilkesiyle aynı çizgi).
...
def parmakizi_cikar(bgr: np.ndarray | None) -> Parmakizi | None:
    ...
    return Parmakizi(renk=renk_izi(kucuk), desen=desen_izi(kucuk))


def renk_izi(bgr: np.ndarray) -> np.ndarray:
    """Yalnız renk histogramı — taramanın UCUZ ön elemesi bunu kullanır."""
...
def desen_izi(bgr: np.ndarray) -> np.ndarray | None:
...
def renk_benzerligi(a: np.ndarray, b: np.ndarray) -> float:
...
def benzerlik(aday: Parmakizi, referans: Parmakizi) -> float:
    ...
    if aday_desensiz and referans_desensiz:
        # İkisi de düz/desensiz: tutunacak tek iz renktir. Bu durumda renk
        # çıtası YÜKSELİR — orta karar bir renk benzerliği "aynı nesne"
        # saydırmaz.
...
def renk_alt_siniri(esik: float) -> float:
    ...
    formülünün üst sınırından çıkarılmıştır, tahmin değildir:

      renk < 0,30  →  en iyi ihtimalle (0,45·renk + 0,55) · 0,65
      renk ≥ 0,30  →  en iyi ihtimalle  0,45·renk + 0,55

    (Desensiz ve uyuşmaz dallarındaki üst sınırlar bunların altında kalır.)
    Eleme yalnızca hızlandırır; sonucu DEĞİŞTİRMEZ — tests/test_nesne_*.py
    bunu rastgele çiftlerle sınar.
    """
    if esik <= _CELISKI_CEZASI * _DESEN_AGIRLIGI:
        return 0.0
    if esik <= _CELISKI_CEZASI * (_RENK_AGIRLIGI * _UYUSMA_ALT_SINIRI + _DESEN_AGIRLIGI):
        return (esik / _CELISKI_CEZASI - _DESEN_AGIRLIGI) / _RENK_AGIRLIGI
    if esik <= _RENK_AGIRLIGI * _UYUSMA_ALT_SINIRI + _DESEN_AGIRLIGI:
        return _UYUSMA_ALT_SINIRI
    return (esik - _DESEN_AGIRLIGI) / _RENK_AGIRLIGI
