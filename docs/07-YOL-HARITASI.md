...
| 27 | **systemd bildirimi** (koşullu, docs/17 §9.3, S1) | Kurulum Docker değil systemd ise `Type=simple` + `WatchdogSec=120` + `NotifyAccess=main` + stdlib `WATCHDOG=1`. Varsayılan Docker dağıtımında yazılmaz; ilk aşamada (24.09.2026) dağıtım sunucu değil fabrikanın Windows bilgisayarıdır (docs/17 §16 S1). Bugün bekçi takılan süreci kapatır (`BEKCI_TEPKISI=yeniden_baslat`, 24.09.2026'dan beri varsayılan; yalnız yeniden açan biri varken): Docker, systemd, (Başlat betiğiyle kurulan sistemde) Kontrol Paneli ya da paketlenmiş Windows uygulamasının gözetmeni yeniden açar (docs/06 §1.2.1, §1.5). | Küçük |
