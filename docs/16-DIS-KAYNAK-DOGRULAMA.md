...
8. **Bekçi (watchdog):** Docker "unhealthy" durumdaki container'ı yeniden başlatmaz. Belgedeki systemd birimi bugün hiç başlamıyor (`app.main:uygulama`). "Takılınca süreçten çık" deseni Docker ve systemd'de işe yarar, ama paketlenmiş masaüstü uygulamasında Kontrol Paneli'ni de öldürür. *24.09.2026:* birim `app.main:app` olarak düzeltildi (`e8e2f98`); bekçi `analiz/bekci.py`'de ve paketlenmiş uygulamada yalnız uyarır (`8f37f73`). *24.09.2026 akşam:* varsayılan tepki `yeniden_baslat`; paketlenmiş Windows uygulamasında paneli gözetmen yeniden açar, Mac uygulamasında bekçi yalnız uyarır (§8 akşam durumu).
...
| Paketlenmiş masaüstü uygulamasında uvicorn Tk Kontrol Paneli ile **aynı süreçte** çalışıyor; `os._exit` paneli de kapatır. Geliştirme kipinde alt süreç çıkınca onu yeniden başlatan bir döngü yok. *24.09.2026:* geliştirme kipinde panel 70 koduyla kapanan sunucuyu yeniden açar (`0b4c09e`); paketlenmiş Windows uygulamasında paneli gözetmen yeniden açar (`970c3d4`). | DOĞRULANDI (yerel) | [dalsan_launcher.py](../masaustu/dalsan_launcher.py):975-979 |
...
*24.09.2026 akşam durumu:* masaüstünde süreci yeniden açan artık var, öneri 2'nin "paketlenmiş masaüstünde süreçten çıkılmaz" kısmı bu yüzden değişti. Başlat betiğiyle kurulan sistemde sunucu Kontrol Paneli'nin alt sürecidir; 70 koduyla kapanınca panel onu yeniden açar, saatte en çok 3 kez (`0b4c09e`). Paketlenmiş Windows uygulamasında çift tıklanan kopya bir gözetmendir (`masaustu/surekli_calisma.py`): sunucuyla aynı süreçteki Kontrol Paneli çökünce, bekçi 70 koduyla kapatınca ya da sunucu beklenmedik şekilde durunca (71) paneli yeniden açar, saatte en çok 3 kez; programı Windows oturumu açılınca başlatır (`970c3d4`). `BEKCI_TEPKISI`'nin varsayılanı `yeniden_baslat` oldu; bekçi süreçten yalnız yeniden açan biri varken çıkar (Docker, systemd, `DALSAN_GOZETMEN=1` veren Kontrol Paneli ya da gözetmen), yoksa (elle çalıştırılan sunucu, Mac uygulaması) yalnız uyarır (`2c73b56`). Operatör 24.09.2026'da ilk aşama dağıtımını fabrikanın Windows bilgisayarı olarak belirledi (sunucusuz; docs/17 §16 S1). Yukarıdaki doğrulanmış bulgular değişmedi: süreçten çıkmak aynı süreçteki paneli yine kapatır, onu gözetmen yeniden açar.

### Açık sorular

- Müşteride `/metrics` ucunu okuyacak bir Prometheus var mı?
- Üretimde hangi dağıtım yolları kullanılacak: Docker, systemd, masaüstü? Masaüstünde süreç çıkarsa onu kim yeniden başlatacak? *(24.09.2026 akşam: ilk aşamada fabrikanın Windows bilgisayarında paketlenmiş uygulama, sunucusuz; masaüstünde yeniden açan Başlat betiğinin Kontrol Paneli ve Windows uygulamasının gözetmenidir, yukarıdaki durum notu. Sunucu aşamasının Docker mı systemd mi olacağı açık.)*
