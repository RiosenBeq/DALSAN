...
| Mevcut model / çerçeve | **YOLOX** (Apache-2.0, ADR-002) `yolox_tiny.onnx` (geliştirme) / `yolox_s.onnx` (fabrika), **ONNX Runtime 1.30.0 CPU paketi** (Intel Mac'te 1.23.2; 22 Eyl'de 1.19.2 idi. `CIKARIM_CIHAZI=cuda` seçeneği var ama `onnxruntime-gpu` depoda hiçbir yerde kurulmuyor - **ölçüldü**, bkz. E12), supervision 0.25.1 ByteTrack; forklift ayrı sınıf DEĞİL (COCO araç sınıfları); KKD sınıflandırıcı henüz eğitilmedi |
| Bluetooth hoparlör | Bugün: yalnız **işletim sistemi eşleştirmesi** + Anons sayfasından ses çıkışı seçimi (Linux'ta kanal satırının `device` alanı, şema 009; eski `ANONS_SES_CIHAZI` ilk açılışta kanala aktarılır); docs/14 §2.1.1 uygulama içi eşleştirmeyi **bilerek yapmamış** ("OS'un işini ikinci kez, daha kötü yapmak"). Cihaz modeli, adedi, mesafe: **operatör dolduracak** |
...
| E12 | §1'de "GPU seçeneği var" varsayımı | **Ölçüldü 22 Eyl:** `onnxruntime==1.19.2` CPU paketidir; `get_available_providers()` → `['AzureExecutionProvider','CPUExecutionProvider']`. `onnxruntime-gpu` ne `requirements.txt`'te ne `Dockerfile`'da kurulu. `docker-compose.yml` (22 Eyl'de satır 37; bugün "GPU" başlıklı yorum bloğu) GPU bloğunu açmayı söylüyor ama imaj GPU'yu kullanamaz | Faz 2 öncesi GPU imajı ayrı kurulmalı; kod düşüşü zaten yakalayıp uyarıyor (22 Eyl'de `tespit.py:124-132`; bugün `Tespitci.__init__` → `cihaz_uyarisi`) |
