...
yarısı, R18'in IP hoparlör kimliği, R28) ve akşamki değişiklik (R5: varsayılan
tepki ve Windows gözetmeni) satırlarında ayrıca yazılıdır (docs/ILERLEME.md,
24.09.2026). Parantezdeki kısaltmalar docs/17 §16'nın açık sorularıdır (S1, S15…).
...
| R5 | düzeltildi | Bekçi analiz iş parçacığının ölümünü ve takılmasını olay, CRITICAL günlük ve `/saglik` sorunu (`analiz_olu`, `analiz_takildi`) olarak bildirir, `BEKCI_TEPKISI=yeniden_baslat` iken süreci kapatır (`8f37f73`). 24.09.2026 akşamından beri bu varsayılandır ve süreç yalnız onu yeniden açan biri varken kapanır (`kaynaklar.yeniden_acan_var_mi`, `2c73b56`): Docker ve systemd; Başlat betiğiyle kurulan sistemde Kontrol Paneli (24.09.2026'dan beri, saatte en çok 3 kez; `dalsan_launcher.py` `surec_kapaninca`); paketlenmiş Windows uygulamasında gözetmen (`masaustu/surekli_calisma.py`, saatte en çok 3 kez, sonra son bir kez yalnız uyararak; `970c3d4`). Yeniden açan yoksa (elle çalıştırılan sunucu, Mac uygulaması) yalnız uyarır. `/saglik` `hazir` verir, `?hazirlik=1` hazır değilse 503 (`e766a69`) |
...
- *Kısmen:* KKD toplu sınıflandırma (karedeki kişiler tek çağrıda, `19d53ef`; kameralar arası değil, piksel eşiği yine kuralda); "otomatik yeniden başlar" (bekçi var, `8f37f73`; süreç yalnız `BEKCI_TEPKISI=yeniden_baslat` iken kapanır, varsayılan `uyar`; 24.09.2026 akşamından beri varsayılan `yeniden_baslat` ve paketlenmiş Windows uygulamasında gözetmen var, §7.4 R5); `egitim/`, `modeller.py` (`backend/app/egitim/` var, `35bae2f`; `modeller.py` yok); Python 3.12 (başlatıcı yalnız 3.12, `becb2e2`; bu konteynerin `.venv`'i 3.11.15).
